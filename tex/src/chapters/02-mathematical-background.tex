...
The time-periodicity assumption is essential: if the solution is not periodic in time, the Fourier representation implicitly extends it periodically, introducing a jump discontinuity at each period boundary. This triggers the \emph{Gibbs phenomenon} \parencite{gibbs1899}, whereby the partial Fourier sums exhibit persistent overshoots near the boundary that do not vanish as the number of modes increases. Since the problems considered here do not exhibit time periodicity, the FNO operating on the full $(x,t)$ domain is expected to suffer from Gibbs artifacts at the temporal boundaries, a limitation that motivates the operator-splitting strategy described in Section~\ref{subsec:pino}.
