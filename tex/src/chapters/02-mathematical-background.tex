...
To bring the Boussinesq system~\eqref{eq:bou1}--\eqref{eq:bou2} into the Fourier domain, we apply the DFT to each equation and use the derivative identities~\eqref{eq:dft_deriv}. The first equation, treating $\widehat{(\eta u)}(k,t)$ as a given nonlinear term, becomes:
...
\begin{equation}\label{eq:error_vector}
    ...
    \end{bmatrix}
    \in \mathbb{R}^{N_\theta},
    \qquad N_\theta = N_0 + N_\mathcal{D}.
\end{equation}
...
\begin{equation}\label{eq:jacobian}
    J(\theta) = \frac{\partial e}{\partial\theta} \in \mathbb{R}^{N_\theta \times P},
    \qquad J_{im} = \frac{\partial e_i}{\partial\theta_m},
\end{equation}
where $P$ is the number of trainable parameters. By the chain rule:
\begin{equation}\label{eq:loss_grad}
    \nabla_\theta\mathcal{L} = J(\theta)^T\,e(\theta) \in \mathbb{R}^P.
\end{equation}
...
\begin{equation}\label{eq:ntk_dynamics}
    \frac{de}{d\tau}
    = -\underbrace{J(\theta)\,J(\theta)^T}_{K(\theta)\;\in\;\mathbb{R}^{N_\theta\times N_\theta}}\,e(\tau).
\end{equation}
...
\end{equation}
with solution $e(\tau) = e^{-K_0 \tau}\,e_0$. Since $K_0$ is symmetric positive semi-definite, it diagonalizes as $K_0 = V\Lambda V^T$ with eigenvalues $\lambda_k \geq 0$. Expanding the initial error in the eigenbasis $e_0 = \sum_k c_k^{(0)} v_k$ decouples~\eqref{eq:error_linear_ode} into $N_\theta$ independent scalar ODEs:
...
\begin{equation}
    e(\tau) = \sum_{k=1}^{N_\theta} c_k(0)\,e^{-\lambda_k \tau}\,v_k.
\end{equation}
...
The NTK analysis explains why this affects high frequencies preferentially. Recall that $K_0 = J_0 J_0^T$, so $K_{0,ij} = J_{0,i} \cdot J_{0,j}$, where each row $J_{0,i} = \nabla_\theta e_i(\theta_0) \in \mathbb{R}^P$ is the parameter-gradient of residual $e_i$ evaluated at spatial location $x_i$. Since $u_\theta$ is a composition of smooth activations applied to affine maps, $J_{0,i}$ varies smoothly as a function of $x_i$.
...
\begin{enumerate}
    \item the rows of $J_0$ are smooth in $x$;
...
\medskip
\noindent\textbf{Extension to time-periodic problems.} When the solution is also assumed to be periodic in time with period $T$ --- so that $D = [0, 2\pi] \times [0, T]$ --- shift-invariance extends to the time direction as well: $\kappa_\ell(x, y, t, \tau) = \kappa_\ell(x - y,\, t - \tau)$. The convolution then generalizes to:
...
The time-periodicity assumption is essential: if the predicted solution at $t = T$ does not coincide with its value at $t = 0$, the Fourier representation implicitly extends the signal periodically, introducing a jump discontinuity of size $\delta = |v(T) - v(0)|$ at each period boundary. For a function with such a jump, the partial Fourier sum $S_K(t) = \sum_{|k_t| \le K} \hat{v}(k_t)\,e^{2\pi i k_t t / T}$ does not converge uniformly near the discontinuity. Instead, as $K \to \infty$, the partial sums overshoot the true value by a fixed fraction of the jump:
\begin{equation}\label{eq:gibbs}
    \lim_{K \to \infty}\Bigl(S_K(t_0^+) - v(t_0^+)\Bigr)
    = \Bigl(\tfrac{\mathrm{Si}(\pi)}{\pi} - \tfrac{1}{2}\Bigr)\delta
    \approx 0.0895\,\delta,
\end{equation}
where $\mathrm{Si}(\pi) = \int_0^\pi \frac{\sin\xi}{\xi}\,d\xi \approx 1.852$. This is the \emph{Gibbs phenomenon}: the ${\approx}9\%$ overshoot persists regardless of $k_{t,\max}$ --- increasing the number of temporal modes only concentrates the oscillations nearer to the boundary without reducing their amplitude. In the FNO, these artifacts propagate through the spectral layers, making the time-periodicity assumption a genuine constraint on the class of problems the architecture handles well.

The FNO training objective is the relative $L^2$ loss:
\begin{equation}
    \mathcal{L}_{\mathrm{FNO}}(\theta) = \frac{1}{N} \sum_{i=1}^N \frac{\| G_\theta(c_i) - u_i \|_{L^2(D)}}{\| u_i \|_{L^2(D)}}.
\end{equation}
...
Since the FNO outputs the predicted fields $(\eta_\theta, u_\theta)$ on a space-time grid of $N_x \times N_t$ nodes, the PDE residuals can be evaluated without automatic differentiation. Spatial derivatives are computed spectrally using $\xi_k$~\eqref{eq:xi_def}: the DFT of $u_\theta$ along the spatial axis at each time $t_n$ yields $\hat{u}_\theta(k, t_n)$, from which
\begin{equation}
    (u_x)_j = \mathcal{F}^{-1}\!\left(i\,\xi_k\,\hat{u}_\theta(k,t_n)\right)_{\!j},
    \qquad
    (u_{xx})_j = \mathcal{F}^{-1}\!\left(-\xi_k^2\,\hat{u}_\theta(k,t_n)\right)_{\!j},
\end{equation}
and identically for $\eta_\theta$. Time derivatives are approximated by finite differences in time: for $\eta_\theta$ sampled at $t_0, \ldots, t_{N_t}$,
\begin{equation}
    (\eta_\theta)_t^n =
    \begin{cases}
        \dfrac{\eta_\theta^{n+1} - \eta_\theta^n}{\Delta t}, & n = 0, \\[8pt]
        \dfrac{\eta_\theta^{n+1} - \eta_\theta^{n-1}}{2\,\Delta t}, & 1 \le n \le N_t - 1, \\[8pt]
...
With all derivatives available, the Boussinesq residuals~\eqref{eq:bou1}--\eqref{eq:bou2} are evaluated at every grid point $(x_j, t_n)$:
\begin{align}
    r_1^{j,n} &= (\eta_\theta)_t + (u_\theta)_x
               + \alpha\,\bigl((\eta_\theta\,u_\theta)_x\bigr), \label{eq:res1} \\
    r_2^{j,n} &= (u_\theta)_t - \tfrac{\beta}{3}(u_\theta)_{xxt}
...
\noindent where:
\begin{itemize}
    \item $\mathcal{L}_{\text{pde}} = \dfrac{1}{N_x N_t}\displaystyle\sum_{j,n}
          \Bigl[\bigl(r_1^{j,n}\bigr)^2 + \bigl(r_2^{j,n}\bigr)^2\Bigr]$
          penalizes violation of the Boussinesq equations at each grid point;

    \item $\mathcal{L}_{\text{ic}} = \dfrac{1}{N_x}\displaystyle\sum_{j}
          \Bigl[(\eta_\theta(x_j,0) - \eta_0(x_j))^2 +
          (u_\theta(x_j,0) - u_0(x_j))^2\Bigr]$
          enforces the initial condition;

    \item $\mathcal{L}_{\text{data}} = \dfrac{1}{N_x N_t}\displaystyle\sum_{j,n}
          \Bigl[(\eta_\theta - \eta_{\mathrm{ref}})_{j,n}^2 +
          (u_\theta - u_{\mathrm{ref}})_{j,n}^2\Bigr]$
          measures deviation from labeled reference solutions;
