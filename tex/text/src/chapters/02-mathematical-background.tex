...
\end{equation}
where $A > 0$ is the initial amplitude and $\operatorname{sech}(z) = 2/(e^z + e^{-z})$ is the hyperbolic secant. This places one crest at the center $x = 0$ of an otherwise flat surface, with zero initial velocity, as drawn in Figure~\ref{fig:soliton_profile}.
...
\begin{equation}\label{eq:wave_eq}
    \eta_{tt} = \eta_{xx}.
\end{equation}
In an analogous manner, differentiating~\eqref{eq:wave2} with respect to $t$ and~\eqref{eq:wave1} with respect to $x$ produces the same cancellation, leaving $u_{tt} - u_{xx} = 0$, so $u$ obeys the same equation.
...
\end{equation}
The initial bump of amplitude $A$ therefore splits into two counter-propagating pulses of amplitude $A/2$, traveling at speed $\pm 1$ without changing shape.
...
\end{equation}
The fundamental identity of the DFT with respect to differentiation is that, for a periodic function, the spatial derivative transforms into a pointwise multiplication \parencite{trefethen2000, steinmoeller2012}:
\begin{equation*}
...
\begin{equation*}
    \hat{Y}(k,t) = \bigl(\hat{\eta}(k,t),\, \hat{u}(k,t)\bigr),
\end{equation*}
the system~\eqref{eq:fourier_ode_k} takes the standard form of an initial value problem for a system of ordinary differential equations,
\begin{equation}\label{eq:ode_form}
    \hat{Y}_t(k, t) = F\bigl(\hat{Y}(k,t)\bigr),
\end{equation}
where the pseudospectral right-hand side operator $F$ is the map
\begin{equation*}
    F\!\left(\hat{Y}(k, t)\right) = F\!\left(\hat{\eta},\hat{u}\right)(k,t)
    \;=\;
    \begin{pmatrix}
        -i\,\xi_k\,\hat{u}(k, t)
        - \alpha\,i\,\xi_k\,\widehat{(\eta u)}(k, t) \\[6pt]
        \dfrac{
            -i\,\xi_k\,\hat{\eta}(k, t)
            - \alpha\,i\,\xi_k\,\widehat{\!\left(\tfrac{1}{2}u^2\right)}\!(k, t)
        }{
    ...
    \end{pmatrix},
\end{equation*}
whose two components are $\hat{\eta}_t$ and $\hat{u}_t$ as given by equations~\eqref{eq:eta_t_spectral}--\eqref{eq:u_t_spectral}, with the nonlinear terms assembled by the pseudospectral procedure above.

In the form~\eqref{eq:ode_form} the problem is what any explicit one-step integrator expects, and time evolution uses the classical fourth-order Runge-Kutta (RK4) scheme \parencite{suli2003, burden2011}. Discretizing~\eqref{eq:ode_form} in time, write $\hat{Y}_n = (\hat{\eta}^n, \hat{u}^n)$ for the spectral state at time $t_n$ and fixed wavenumber $k$. The four stage evaluations $\kappa_1, \kappa_2, \kappa_3, \kappa_4$ and the update to $t_{n+1} = t_n + \Delta t$ are:
\begin{align*}
    \kappa_1 &= F(\hat{Y}_n), \\
...
The numerical experiments in this work vary only the PDE parameters $(\alpha,\beta)$ while keeping the initial condition fixed at~\eqref{eq:initial_condition}. The methods are therefore trained and evaluated on the dataset
...
\begin{align*}
    h^{(0)}       &= x \in \mathbb{R}^{d_{\mathrm{in}}},                                                                  \\
    h^{(l)}       &= \sigma \left( W^{(l)} h^{(l-1)} + b^{(l)} \right) \in \mathbb{R}^{N_{\mathrm{MLP}}},
                   \quad l = 1, \dots, L_{\mathrm{MLP}}-1,  \\
    u_{\theta}(x) &= W^{(L_{\mathrm{MLP}})} h^{(L_{\mathrm{MLP}}-1)} + b^{(L_{\mathrm{MLP}})} \in \mathbb{R}^{d_{\mathrm{out}}}.
\end{align*}
\noindent where:
\begin{itemize}
...
Introduced by \textcite{raissi2019}, Physics-Informed Neural Networks (PINNs) insert the PDE directly into the optimization process, forcing the neural network to satisfy the model equations and to approximate the PDE solution.
...
In this work, $G^\dagger$ is the solution operator of the Boussinesq system: given an input $a$ encoding the PDE coefficients $(\alpha, \beta)$ together with the initial condition~\eqref{eq:initial_condition}, the operator returns the full space-time solution $(\eta, u)$. Because the initial condition is the same for every sample of the dataset~\eqref{eq:dataset}, it carries nothing that distinguishes one sample from another, and the input reduces to the pair $(\alpha_i, \beta_i)$, which is how the operator is written from here on. We note that there would be no problem to consider the initial condition (or boundary conditions and forcings) as the PDE parameter to be mapped to the corresponding solution.
...
\end{align}
Here $(\eta_\theta^i, u_\theta^i) \coloneqq G_\theta(\alpha_i, \beta_i)$ denotes the network output for sample $i$. The PINO loss is a weighted sum of three terms:
...
\begin{itemize}
    \item $\mathcal{L}_{\text{pde}}(\theta) = \dfrac{1}{M}\displaystyle\sum_{i=1}^{M}\dfrac{1}{N_x N_t}\displaystyle\sum_{j=0}^{N_x-1}\sum_{n=0}^{N_t}
          ...
          penalizes violation of the Boussinesq equations at each grid point, with residuals evaluated using the parameters $(\alpha_i,\beta_i)$ of each sample and the subscript $_{j,n}$ indicates evaluation at the grid point $(x_j, t_n)$;
    ...
    \item $\mathcal{L}_{\text{data}}(\theta) = \dfrac{1}{M}\displaystyle\sum_{i=1}^{M}\dfrac{1}{N_x N_t}\displaystyle\sum_{j=0}^{N_x-1}\sum_{n=0}^{N_t}
          \Bigl[(\eta_\theta^i(x_j,t_n) - \eta_i(x_j,t_n))^2 +
          (u_\theta^i(x_j,t_n) - u_i(x_j,t_n))^2\Bigr]$
          measures deviation from the reference solutions $(\eta_i,u_i) \in \mathcal{S}$~\eqref{eq:dataset};
