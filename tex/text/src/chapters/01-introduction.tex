...
Physics-Informed Neural Operators (PINOs). A plain multilayer perceptron (MLP), trained on data alone, serves as a baseline and as the testbed for the spectral-bias analysis. All results are measured against a
...
\begin{itemize}
    ...
    \item The mechanism behind spectral bias: does Neural Tangent Kernel
          ...
          quantitative tests on a minimal network fitting a single Boussinesq elevation profile, across a range of network widths. The
          first targets the initial-kernel (lazy-training) assumption the analysis
