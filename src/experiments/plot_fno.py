...
def eval_fno(model_metadata_file, x_limit, t_limit, eval_params, resolutions, spectral_res, output_dir=None):
    ...
    if norm_stats is None:
        raise RuntimeError('Model metadata must contain norm_stats for consistent FNO evaluation.')
    norm_stats = {
    ...
    alpha_true_list = []
    alpha_pred_list = []
    print('start fno evaluation alpha/beta panel')
    for val in eval_params:
        ...
        pred = model.predict_physical(input_tensor, norm_stats)
    ...
    plot_model2_alpha_beta_panel(
        ...
        title=f'FNO Alpha-Beta Panel (res {int(median_res)})',
    ...
    res_true_list = []
    res_pred_list = []
    print('start fno evaluation resolution panel')
    ...
    plot_model2_resolution_panel(
        ...
        title=f'FNO Resolution Panel (alpha=beta {median_param:.3f})',
        param_label=f'{median_param:.3f}',
    )

    print('start fno evaluation spectral panel')
    bsq = Boussinesq(-x_limit, x_limit, 0, t_limit, median_param, median_param, 1)
    ...
    plot_model2_spectral_panel(
        ...
        title=f'FNO Spectral Panel (alpha=beta {median_param:.3f}, res {int(spectral_res)})',
        param_label=f'{median_param:.3f}',
...
def gif_fno(model_metadata_file, x_limit, t_limit, params, resolution, outdir):
    # one gif per parameter: eta(x,t) evolving in time, reference vs FNO prediction
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_metadata = torch.load(model_metadata_file, map_location='cpu')
    ...
    model = FNO2d(modes1=p['modes1'], modes2=p['modes2'], width=p['width']).to(device)
    load_model(model_file, model, device=device)
    model.eval()
    ...
    for val in params:
        ...
        save_solution_gif(
            ...
            outdir=outdir,
            filename=f'fno_{val:.2f}.gif',
            title_prefix=rf'FNO  $\alpha=\beta={val:.2f}$',
        )